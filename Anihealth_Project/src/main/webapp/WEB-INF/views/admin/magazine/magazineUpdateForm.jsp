...
<body>
	...
	<div class="outer">
        ...
        <form id="postForm" action="updateMaga.ad" method="post" enctype="multipart/form-data">
        	...
        	<div class="detail-area">
	            ...
	            <table align="center">
	            	<tr>
	            		...
	            		<td width="600px"><input type="text" class="form-control col-lg" name="magazineTitle" id="magazineTitle" value="${ requestScope.ma.magazineTitle }" required></td>
	            	</tr>
	            </table>
	            <br>
	            <div class="gray-line"></div>
	            ...
	            <table id="magazineContent" class="table">
	            		...
	            		<c:forEach var="m" items="${ requestScope.list }" varStatus="status">
	            			<c:choose>
	            				<c:when test="${ m.magazineImgLevel eq 1 }">
	            					<input type="hidden" name="magazineAttachmentNo" value="${ m.magazineAttachmentNo }">
	            					<input type="hidden" name="beforePath" value="${ m.magazinePath }">
	            					<tr>
		            					<th>
					            			썸네일이미지 <br> <p style="font-size : 0.9em; color : red;">* 필수</p>
					            		</th>
					            		<td>
					            			<img name="thumbnail" class="imgContent" id="thumbnail" src="${ m.magazinePath }">
					            		</td>
					            		<td>
					            			<textarea name="magazineContent" class="form-control content" cols="60" rows="18" style="resize: none;" required>${ m.magazineContent}</textarea>
					            		</td>
					            	</tr>
	            				</c:when>
	            				<c:otherwise>
	            					...
	            					<tr>
					            		...
					            		<td>
					            			<img name="detail" class="detail imgContent" src="${ m.magazinePath }">
					            		</td>
					            		<td>
					            			<textarea name="magazineContent" class="form-control content" cols="60" rows="18" style="resize: none;" required>${ m.magazineContent}</textarea>
					            		</td>
        	...
        	<div id="file-area" style="display : none;">
        		<c:forEach var="f" items="${ requestScope.list }">
        			<input type="file" name="files" onchange="loadImg(this, ${ list.indexOf(f) });">
        		</c:forEach>
	        </div>
	        
	        <div class="btns" align="center">
	        	<button class="btn btn-lg" type="submit">수정하기</button>
	        	<a class="btn btn-lg" href="detailMaga.ad?mno=${ requestScope.ma.magazineNo }">뒤로가기</a>
        ...
        <script>
        	// 매거진 내용 영역 추가 및 삭제
        	<c:set var="listSize" value="${fn:length(requestScope.list)}" />
        	let fileCount = "${listSize}" - 1;
